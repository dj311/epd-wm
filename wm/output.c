...
)
{
  ...
  wlr_output_layout_output_coords(output->server->output_layout, wlr_output,
                                  &x, &y);
...
)
{
  ...
  wl_signal_add(&server->output->damage->events.frame,
                &server->output->damage_frame);
  server->output->damage_destroy.notify = handle_output_damage_destroy;
  wl_signal_add(&server->output->damage->events.destroy,
                &server->output->damage_destroy);
  ...
  if (wlr_xcursor_manager_load
      (server->seat->xcursor_manager, wlr_output->scale)) {
    wlr_log(WLR_ERROR,
...
void
output_damage_view_surface(
  struct cg_output *cg_output,
  struct cg_view *view
)
{
